...
const dispatch = useAppDispatch();
  ...
  return (
    <>
      ...
      {cartItems.length > 0 ? (
        <section className="overflow-hidden py-20 bg-[#1e1e1e]">
          <div className="max-w-[1170px] w-full mx-auto px-4 sm:px-8 xl:px-0">
            <div className="flex flex-wrap items-center justify-between gap-5 mb-7.5">
              <h2 className="font-medium text-white  text-2xl">Your Cart</h2>
              <button  onClick={() => dispatch(removeAllItemsFromCart())} className="text-[var(--gold)]">Clear Shopping Cart</button>
            </div>
        ...
        <>
          <div className="text-center pb-25 bg-[#1e1e1e]">
            ...
            <p className="pb-6 text-center text-white">Your cart is empty!</p>

            <Link
              href="/shop-with-sidebar"
              className="w-96 mx-auto flex justify-center font-medium text-white bg-[#F5BE32] py-[13px] px-6 rounded-md ease-out duration-200 hover:bg-[var(--gold)]"
            >
              Continue Shopping
            </Link>
          </div>
