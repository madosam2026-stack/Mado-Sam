...
const ProductItem = ({ item }: { item: Product }) => {
  ...
  return (
    <div className="group">
      <div className="relative overflow-hidden flex items-center justify-center rounded-lg bg-[#F6F7FB] min-h-[270px] mb-4">
        ...
        <div className="absolute left-0 bottom-0 translate-y-full w-full flex items-center justify-center gap-2.5 pb-5 ease-linear duration-200 group-hover:translate-y-0">
          <button
            ...
            className="flex items-center justify-center w-9 h-9 rounded-[5px] shadow-1 ease-out duration-200 text-[#000] bg-white hover:text-[var(--gold)]"
          >
            <svg
          ...
          <button
            ...
            className="inline-flex font-medium text-custom-sm py-[7px] px-5 rounded-[5px] bg-[var(--gold)] text-white ease-out duration-200 hover:bg-yellow-light"
          >
            Add to cart
          </button>
          ...
          <button
            ...
            id="favOne"
            className="flex items-center justify-center w-9 h-9 rounded-[5px] shadow-1 ease-out duration-200  bg-white text-[#000]  hover:text-[var(--gold)]"
          >
            <svg
              className="fill-current"
              width="16"
      ...
      <h3
        className="font-medium text-white ease-out duration-200 mb-1.5"
      ...
      <span className="flex items-center gap-2 font-medium text-lg">
        <span className="text-[var(--gold)]">AED {item.discountedPrice}</span>
{item.gm > 0 && (
  <span className="text-white">
    {item.gm >= 1000 ? `${item.gm / 1000} kg` : `${item.gm} gm`}
  </span>
)}
          {/* <span className="text-[var(--gold)] line-through">${item.price}</span>  */}
      </span>
    </div>
  );
};
